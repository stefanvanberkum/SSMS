...
def plot_states(results: MLEResults, regions: list, z_names: list, save_path: str):
    ...
    for i in range(n_betas):
        ...
        p = ggplot(states, aes(x='Date')) \
            ...
            + labs(x='Date', y=states.columns[n_regions*2+i], color='Legend')
        # print(p)
        ggsave(plot=p, filename=states.columns[n_regions*2+i], path=save_path, verbose=False, dpi=600)
...
def forecast_error(results: MLEResults, model, regions: list, save_path: str, first=int, last=int, ci=bool, tp=str):
    ...
    for region in range(len(regions)):
        ...
        mae_naive = np.mean(np.abs([x - model.endog[first:, region][i - 1] for i, x in enumerate(model.endog[first:, region])][1:]))
